...
def displace_func(noiseVal, input_filename):
    ...
    noise_layers = {
        'layer1': [PerlinNoise(octaves=1), PerlinNoise(octaves=1)], # x and y noise for layer 1
        'layer2': [PerlinNoise(octaves=4), PerlinNoise(octaves=4)], # x and y noise for layer 2
        'layer3': [PerlinNoise(octaves=2), PerlinNoise(octaves=20)]  # x and y noise for layer 3
    }
    ...
    def displace_point(pt, scales, noise_scales):
        ...
        for i, (noise_x, noise_y) in enumerate(noise_layers.values()):
            dx += noise_x([pt.real * noise_scales[i], pt.imag * noise_scales[i]]) * scales[i][0]
            dy += noise_y([pt.imag * noise_scales[i], pt.real * noise_scales[i]]) * scales[i][1]
    ...
    def displace_path(path, scales, noise_scales):
        ...
        for segment in path:
            if isinstance(segment, (svg.Line, svg.CubicBezier)):
                points = [segment.start, segment.control1, segment.control2, segment.end] if isinstance(segment, svg.CubicBezier) else [segment.start, segment.end]
                displaced_points = [displace_point(pt, scales, noise_scales) for pt in points]
                if isinstance(segment, svg.Line):
                    displaced_segment = svg.Line(displaced_points[0], displaced_points[1])
                else:
                    displaced_segment = svg.CubicBezier(displaced_points[0], displaced_points[1], displaced_points[2], displaced_points[3])
            else:
    ...
    def process_file(input_filename, output_suffix, scales, noise_scales):
        ...
        svg.wsvg(displaced_paths, attributes=attributes, svg_attributes=svg_attributes, filename=output_filename)
    ...
    svg.wsvg(displaced_paths, attributes=attributes, svg_attributes=svg_attributes, filename=output_filename_test_lines)
...
def _draw( strokes, lines, filename, stroke_colors=None, stroke_widths=None, dither= 0,line_height = 30, margins=50, line_break= 0.5 ,noiseVal = 1):
        ...
        scale = min((desired_width - (2 *  margins))/ svg_width, (desired_height - (2 *  margins)) / svg_height)
        ...
        originalSVG.moveto(margins,margins)
        
        

        # Create a new SVG figure
        figure = svgutils.compose.Figure(desired_width, desired_height, originalSVG)
